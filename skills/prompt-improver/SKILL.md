...
| `fast_path.mode` / `PROMPT_IMPROVER_FAST_PATH` | **Experimental**, default `off`. `auto` uses Jev (TypeSafe System One, needs `TYPESAFE_API_KEY` or `OPENROUTER_API_KEY`) to compile the spec from a reviewed library when a recipe fits (no LLM call), to have a fast model write only the missing sections, or to add the repository facts it selected to the normal generation prompt. `ground` only does the last. Sends the redacted request and repository facts to TypeSafe/OpenRouter. Any Jev failure falls back silently. The host flow and exit codes are unchanged. |
