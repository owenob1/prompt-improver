...
The worker replaces the earlier `prompt-improver-mcp`, which used Workers AI. That worker bound a `PromptSession` Durable Object, so this one still exports a no-op class of that name. This lets the deploy go through without a class-deletion migration. Remove it later with a `deleted_classes: ["PromptSession"]` migration.
